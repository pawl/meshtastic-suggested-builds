...
- **Battery**: [2000mAh H2T battery from muzi.works](https://muzi.works/products/h2t-battery) (8 x 31 x 61.5mm), upgraded from the original 1200mAh h1-battery. Straight swap, same 1.25mm connector, fits the H2T case with GPS.
...
- Buffer LiPo (have it): MakerFocus 3000mAh flat cell with a JST PH 2.0 plug already wired for RAK polarity. The 1W radio can't run off the base board's 3.3V rail and needs a battery on the board to absorb the transmit surge. Just having this connected is what unlocks full 1W. Chose 3000mAh on purpose: the base board charges a connected cell slowly, so a smaller buffer stays reliably topped, while the 18650s stay the main storage. (This replaces the earlier plan to reuse the 1800mAh muzi 604050 cell; that cell is freed up and is too wide for the T114 cases anyway. The MakerFocus is worth $13 to skip the connector re-termination and polarity work.)
- Power wiring change: feed the Waveshare 5V output into the booster's EX_5V input, not the base-board Solar-In as before, with the module power jumper set to EX_5V. EX_5V powers the whole stack and keeps the buffer charged. The 5V lead for this comes in the kit. Move the 18650 pack onto the Waveshare BAT+/BAT- terminals (screw terminal or PH2.0 4P). In the current build the pack is plugged into the RAK battery port and the Waveshare BAT terminals are empty, which means the Waveshare only produces 5V in daylight. With the pack on the Waveshare, 5V is available around the clock and the module's 3.0V over-discharge cutoff protects the cells. The RAK battery port now holds the buffer LiPo instead. Sticking with the existing 3-cell holder: ~38Wh is already close to a week of autonomy at this load, so a 4-cell adds little. Still do NOT charge through the base-board USB-C port.
...
- New enclosure (have it): LMioEtool 220x170x110mm (8.7 x 6.7 x 4.3 inch), grey opaque cover, with the removable mounting plate. This is the larger size of the same LMioEtool line as the current box. The old 150x100x70 was a tight fit even for the RAK4631, and won't hold the longer 1W board plus the buffer cell with room to secure anything. Grey opaque, not clear: a clear lid on a Dallas roof is a greenhouse. The removable plate is the other reason to switch, since everything mounts on the bench and drops in, so drilling happens in an empty shell. The existing pole-mount tabs and hose-clamp bar transfer over to this box.
- Cable entry: retire the USB-C flush-mount pass-through. The panel now wires straight into the Waveshare, so replace it with a PG7 cable gland (from a PG7/PG9/PG11 assortment); the Voltaic lead falls in PG7's 3 to 6.5mm clamp range. Mount the gland pointing down so water sheds off it. Two holes total in the new box, both on the bottom face since the antenna is now remote: the N bulkhead and the gland.
- Internal mounting: screw-down saddle cable-tie mounts (black nylon, screw type not adhesive) fixed to the new plate, instead of relying on the grid. Adhesive-backed mounts let go in rooftop heat, so avoid those. Tie the 18650 holder and the buffer cell to the shaded lower area, not against the 1W module. Skip the printed Waveshare-over-WisBlock stacking adapter in this build: it existed to save space in the small box, and the 1W module runs hotter, so mount the two boards side by side on the plate instead.

Panel placement and heat (Dallas):
- Mount the antenna separately on the mast using the U-bolt bracket it shipped with, above the panel, with the box directly beneath the panel. This is what lets the panel shade the box at midday, which matters more in Dallas than the feedline loss it adds (a short jumper in RG-8X or LMR-240 grade costs roughly 0.3 to 0.4 dB including connectors). Tilt the panel about 35 degrees facing south; at summer noon the sun is nearly overhead, so the shadow drops almost straight down onto the box. Leave a drip loop in the coax below the bulkhead. Note the antenna is N-male, so the jumper needs N-male at the box bulkhead and N-female at the antenna.
...
Carries over from the current build: Waveshare Solar Power Manager D, the 18650 LG MJ1 cells and 3-cell holder, the 5.8 dBi N-male antenna, and the spare U.FL to N pigtail (the RAK13302 uses an IPEX connector, so it fits).
